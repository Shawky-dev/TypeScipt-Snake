...
let ScoreTag = document.getElementById("score") as HTMLParagraphElement
let GameOverTag = document.getElementById("gameover") as HTMLDivElement
let Score: number = 0
canvas.height = window.innerHeight * 0.6
canvas.width = window.innerWidth * 0.6
let ctx = canvas.getContext("2d") as CanvasRenderingContext2D
const cellSize: number = 20
const cellBorders: number = 1
class Player {
  Score: number = -3
  bodyParts: BodyPart[] = [new BodyPart(200, 200)]
  direction: string = "d"
  draw(): void {
  ...
  advanceSnake(): void {
    let head: BodyPart = new BodyPart(
      this.bodyParts[0].x + cellSize * this.getDirectionX(),
      this.bodyParts[0].y + cellSize * this.getDirectionY()
    )
...
class Food {
  food_x!: number
  food_y!: number
  random_food(min: number, max: number): number {
  ...
  has_eaten_food(player: Player): void {
...
let food1 = new Food()
food1.gen_food()
let has_game_ended = (): boolean => {
  for (let i = 4; i < player1.bodyParts.length; i++) {
    if (
      player1.bodyParts[i].x === player1.bodyParts[0].x &&
...
let areRectsColliding = (player: Player, food: Food): boolean => {
  const size = cellSize
  return !(
    player.bodyParts[0].x + size <= food.food_x ||
